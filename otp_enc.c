...
Description: This program accepts a file containing text, along with a file
	containing a key to encrypt that text, and passes those files via a socket
	to a server, so that they may be encrypted. Upon receipt, the encoded text
	is printed.
*/
...
int main(int argc, char *argv[])
{
    ...
    int plaintext;							/*File descriptor for plaintext file*/
    int key;								/*File descriptor for key file*/
    int plaintextLength;					/*Length of plaintext char array*/
	...
	char validate[] = "ACCEPT_ENC_CLIENT";
	
	/*Open the text files, passed in via command line*/
	if((plaintext = open(argv[1], O_RDONLY)) == -1)
	{
		perror("open");
		exit(1);	
	}
	...
	plaintextLength = validateInput(plaintext);
	keyLength = validateInput(key);
	
	/*Verify that files do not contain invalid input*/
	if (plaintextLength == -1 || keyLength == -1)
	{
		fprintf(stderr,"%s error: input contains bad characters\n",argv[0]);
		exit(1);
	}
	
	/*Checks that key is at least as long as the plaintext file*/
    ...
    if (strcmp(validate, "ACCEPT_ENC_SERVER") != 0)
	{
//        perror("port mismatch");
        exit(1);
    }
	
	/*Loop runs until entire plaintext file is read and translated*/
	while(1)
	{
		/*numBytes returns the number of bytes read from the plaintext file*/
		if((numBytes = read(plaintext, plaintextBuffer, 255)) == -1)
		{
			perror("read");
			exit(1);
		}
		...
		/*Write the corresponding plaintext and key chunks to the server*/
		if(write(sockfd, plaintextBuffer, numBytes) == -1)
		{
			perror("write");
			exit(1);
		}
		...
		/*Server returns the encrypted text*/
		if(read(sockfd, ciphertextBuffer, numBytes) == -1)
		{
			perror("read");
			exit(1);
		}
		...
		/*This is our encoded text*/
		printf("%s",ciphertextBuffer);
