...
Description: This program is a daemon (runs in the background as a server) to
	accept files from client processes containing text. It reads in unencrypted
	text, and a randomly generated key. Using the key as a one-time pad, it
	encrypts the text and returns it to the client.
*/
...
int main(int argc, char *argv[])
{
	if(argc != 2)
	{
		fprintf(stderr, "Usage: otp_enc_c portNumber\n");
		exit(1);
	}
    ...
    int pid;
    int status;
	struct sockaddr_in server;		/*Info about the server's connection*/
	...
	char buffer[256];
	char plaintextBuffer[256];					/*Holds chunks of the plaintext sent from client*/
	char keyBuffer[256];						/*Holds corresponding chunks of the key, sent from client*/
	char *ciphertextBuffer;						/*Points to plaintextBuffer after encryption, given separate name for clarity*/
	...
	char validate[] = "ACCEPT_ENC_SERVER";
    ...
    while (1)
	{
		...
		{	
            ...
            if (strcmp(buffer, "ACCEPT_ENC_CLIENT") != 0)
			{
                printf("Error: %s cannot find otp_dec_d",argv[0]);
                strcpy(validate, "REJECT_ENC_SERVER");
			...
			while(1)
			{
				...
				if((numBytes = read(newsockfd, plaintextBuffer, 255)) == -1)
				{
					perror("read");
					exit(1);
				}
				...
				/*The corresponding sections of text and key are sent to be encoded,
				and the encoded text is returned.*/
				ciphertextBuffer = encrypt(plaintextBuffer, keyBuffer, numBytes);
				
				/*Write the encoded text back to the client*/
				if(write(newsockfd, ciphertextBuffer, numBytes) == -1)
				{
					perror("write");
					exit(1);
				}
...
/*Takes a length of human-readable text, and an equal length of pseudo-randomly
generated cipher, and converts it into encoded text. Writes the cipher "in-place",
the unencrypted text is replaced a letter at a time in the original char array,
just because it seems cool in my mind to have the text transform into code right
before your eyes.*/
char *encrypt(char *plaintext, char *key, int length)
{
	int i;
	for (i = 0; i < length; i++)
	{
		/*Convert the plaintext to values 0 - 26, where A = 0, B = 1 ... Z = 25, ' ' = 26*/
		if(plaintext[i] == ' ')
		{
			plaintext[i] = 26;
		}
		...
		/*The cipher is add the key and plaintext, and mod the value*/
		plaintext[i] = (plaintext[i] + key[i]) % 27;
